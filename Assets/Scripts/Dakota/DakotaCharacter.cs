...
namespace Dakota
{
    public class DakotaCharacter : MonoBehaviour
    {
        ...
        [SerializeField] private bool m_CanDoubleJump = false;                  // Dakota's default Z rotation
        [SerializeField] private float m_DoubleJumpForce = 200f;                  // Amount of force added when the player jumps.
        ...
        [SerializeField] private int m_IdleThreshold = 100;                  // Idle time until Dakota will sit
        [SerializeField] private float m_DefaultRotation = 0.0f;                  // Dakota's default Z rotation
        [SerializeField] private bool m_CanAttack = true;                  // Dakota's default Z rotation
        [SerializeField] private bool m_CanShoot = true;                  // Dakota's default Z rotation
        [SerializeField] private float m_AttackDmg = 10.0f;                  // Dakota's default Z rotation
        [SerializeField] private float m_ProjectileDmg = 2.0f;                  // Dakota's default Z rotation
        [SerializeField] private GameObject m_ProjectileObject;                  // Dakota's default Z rotation
        [SerializeField] private Transform m_ShootingPoint;                  // Dakota's default Z rotation
        ...
        public void Attack() {
            ...
            RaycastHit2D hit = Physics2D.Raycast(m_HoldPoint.position, Vector2.right * transform.localScale.x, m_Reach);
            if(hit.collider != null && hit.collider.tag == "Grabbable") {
                Destructible script = hit.transform.gameObject.GetComponent<Destructible>();
                if (script != null) {
                    // Inflict damage
                    script.Hit(m_ProjectileDmg);
                } 
            }
        ...
        public void Shoot() {
            ...
            // TODO: Need to apply some velocity to it!
